define(['jquery', 'knockout', 'text!static/pages/yzqc/mbe_grpmg/mbe_grpmg_list.html'], function ($, ko, template) {

        //跳转信息
        var infoUrl = "/yzqc/mbe_grpmg/mbe_grpmg_detail/:pkgrpmg";

        var addUrl = '/yzqc/mbe_grpmg/mbe_grpmg_detail/0';

        var deleteUrl = '/mbe_grpmg/delete/';

        var pageUrl = '/mbe_grpmg/page';
        ...
        viewModel.del = function () {
            console.info($ctx + deleteUrl + this.pkMbeGroupmg + ".json");
            if (confirm("确认删除这条数据吗?")) {
                $.ajax({
                    type: 'DELETE',
                    async: false,
                    url: $ctx + deleteUrl + this.pkMbeGroupmg + ".json",
